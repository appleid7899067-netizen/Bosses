...
export async function runFleet(data: FleetRequest, onDelta?: (full: string) => void, onActivity?: (activity: string[]) => void): Promise<ChatResult> {
  ...
  // Provider routing:
  // - OpenRouter key connected => prefer OpenRouter (no Puter login required)
  // - Explicit puter:<model> stays on Puter
  // - Otherwise fall through to Puter free models when signed in
  const selectedModelId = (data.modelId || "").trim();
  const isPuterSelection = /^puter:/i.test(selectedModelId);
  const isOpenRouterSelection = /^openrouter:/i.test(selectedModelId) || (Boolean(activeKey) && !isPuterSelection);
  if (isOpenRouterSelection) {
    try {
      ...
      if (!selected) return { ok: false, error: "OpenRouter key ใช้งานได้ แต่ยังไม่มีโมเดลที่เลือกหรือไม่มีโมเดลข้อความที่ OpenRouter เปิดให้บัญชีนี้เรียกได้" };
      ...
      if (openRouterResult.ok) {
        const activity = ["🔑 ตรวจ OpenRouter API key", "📚 อ่าน OpenRouter model catalog", `🏷️ Gateway: OpenRouter`, `🧭 OpenRouter model: ${selected.id}`, "⚙️ เรียกโมเดลผ่าน OpenRouter", "✅ ส่งผลลัพธ์"];
        onActivity?.(activity);
