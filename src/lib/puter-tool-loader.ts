...
function nativeGitSearchTools(): CodingFleetTool[] {
  ...
  return [
    search("github_search_repositories", "Search GitHub repositories.", "repo:owner/name, user:owner, language:typescript"),
    search("github_search_code", "Search source code.", "repo:owner/name path:src 502"),
    search("github_search_commits", "Search commits.", "repo:owner/name fix 502"),
    search("github_search_issues", "Search issues.", "repo:owner/name is:open bug"),
    search("github_search_prs", "Search Pull Requests.", "repo:owner/name is:pr is:open"),
    search("github_search_branches", "Search branches.", "owner/name feature")
  ];
}
...
async function executeGitHubTool(tool: CodingFleetTool, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (toolName(tool).startsWith("github_search_")) { const kind = toolName(tool).replace("github_search_", ""); const endpoint = kind === "repositories" ? "/search/repositories" : kind === "code" ? "/search/code" : kind === "commits" ? "/search/commits" : kind === "issues" || kind === "prs" ? "/search/issues" : "/search/branches"; const query = String(args.q ?? "").trim(); if (!query) throw new Error("GitHub search query is required."); path = endpoint + `?q=${encodeURIComponent(query)}&per_page=${Math.min(20, Math.max(1, Number(args.per_page ?? 10)))}`; } else if (toolName(tool) === "github_get_file") {
